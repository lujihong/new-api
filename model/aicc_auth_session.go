...
func RecordAICCAuthSession(userID int, token string, expiresIn int, channelID ...int) error {
	...
	session := AICCAuthSession{UserID: userID, TokenHash: aiccTokenHash(token), ChannelID: cID, ExpiresAt: time.Now().Add(time.Duration(expiresIn) * time.Second)}
...
func GetAICCAuthSessionChannelID(userID int, token string) (int, error) {
	...
	err := DB.Select("channel_id").Where("user_id = ? AND token_hash = ?", userID, aiccTokenHash(token)).First(&session).Error
	if err != nil {
		return 0, err
	}
