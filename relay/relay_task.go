...
func prepareTaskSubmission(c *gin.Context, info *relaycommon.RelayInfo) (adaptor channel.TaskAdaptor, platform constant.TaskPlatform, taskErr *dto.TaskError) {
	...
	if useTiered {
		...
		cost, trace, runErr := billingexpr.RunExprWithRequest(exprStr, billingexpr.TokenParams{}, billingexpr.RequestInput{Body: requestBody, Usage: facts})
...
func videoFetchByIDRespBodyBuilder(c *gin.Context) (respBody []byte, taskResp *dto.TaskError) {
	...
	isOpenAIVideoAPI := strings.HasPrefix(c.Request.RequestURI, "/v1/videos/") || strings.HasPrefix(c.Request.RequestURI, "/v1/video/generations/") || strings.Contains(c.Request.RequestURI, "/contents/generations/tasks")
...
func TaskModel2Dto(task *model.Task) *dto.TaskDto {
	...
	if task.Status == model.TaskStatusSuccess && task.TaskID != "" {
