...
export function getAvailableGroups(
  ...
  return Object.keys(usableGroup)
    ...
    .filter((g) => modelEnableGroups.includes('all') || modelEnableGroups.includes(g))
}
...
/** Server returns the final multiplier: do not multiply the discount again. */
export function getModelGroupRatio(
  model: PricingModel,
  group: string,
  fallback: Record<string, number> = model.group_ratio ?? {}
): number {
  return getConfiguredGroupRatio(
    { ...fallback, ...model.model_group_ratio },
    group
  )
}
...
export function getDisplayGroupRatio(
  ...
  const modelEnableGroups = Array.isArray(model.enable_groups)
    ? model.enable_groups
    : []
  const groupRatio = { ...model.group_ratio, ...model.model_group_ratio }

  if (
    selectedGroup &&
    selectedGroup !== FILTER_ALL &&
    (modelEnableGroups.includes('all') || modelEnableGroups.includes(selectedGroup))
  ) {
  ...
  const candidateGroups = modelEnableGroups.includes('all') ? Object.keys(groupRatio) : modelEnableGroups
