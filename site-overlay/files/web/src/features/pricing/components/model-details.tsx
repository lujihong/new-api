...
import { getAvailableGroups, getDisplayGroupRatio, getModelGroupRatio, isTokenBasedModel } from '../lib/model-helpers'
import { ModelDiscountCaption } from './model-discount-caption'
...
function PriceSection(props: {
  ...
  const viewerRatio = props.model.model_group_ratio ? getDisplayGroupRatio(props.model) : 1
  const baseGroupRatioMap = { [baseGroupKey]: viewerRatio }
  const dynamicSummary = getDynamicPricingSummary(props.model, {
    tokenUnit: props.tokenUnit,
    showRechargePrice: props.showRechargePrice,
    priceRate: props.priceRate,
    usdExchangeRate: props.usdExchangeRate,
    groupRatioMultiplier: viewerRatio,
  })
  ...
  if (dynamicSummary) {
    if (dynamicSummary.isSpecialExpression) {
      return (
        <section>
          <SectionTitle>{props.model.model_group_ratio ? t('Your price', { defaultValue: '本人优惠价' }) : t('Base Price')}</SectionTitle>
  ...
  if (isDynamicPricingModel(props.model)) {
    ...
    const formattedPricesByGroup = new Map(
      availableGroups.map((group) => {
        const ratio = getModelGroupRatio(props.model, group, props.groupRatio)
        return [
          group,
      ...
      <StaticDataTable
        ...
        columns={[
          ...
          },
          {
            ...
            cell: (group) => `${getModelGroupRatio(props.model, group, props.groupRatio)}x`,
          },
          ...(isTokenBased
            ? [
                {
          ...
          <section className='bg-card/60 space-y-5 rounded-xl border p-4 shadow-sm'>
            ...
            {isDynamic && !simpleTaskPricing && (
              <DynamicPricingBreakdown
                ...
                usageSchema={props.model.billing_usage_schema}
                taskPriceOptions={{
                  groupRatioMultiplier: props.model.model_group_ratio ? getDisplayGroupRatio(props.model) : 1,
                  showRechargePrice,
                  priceRate: props.priceRate,
                  usdExchangeRate: props.usdExchangeRate,
                }}
