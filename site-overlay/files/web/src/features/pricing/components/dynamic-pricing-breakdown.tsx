...
type DynamicPricingBreakdownProps = {
  ...
  taskPriceOptions?: Pick<
    DynamicPriceOptions,
    'showRechargePrice' | 'priceRate' | 'usdExchangeRate' | 'groupRatioMultiplier'
  >
  /**
...
function formatBreakdownConditionSummary(
  ...
  if (!isTaskBreakdownTier(tier)) {
    ...
    if ('conditionSummary' in tier && typeof tier.conditionSummary === 'string' && tier.conditionSummary) {
      return tier.conditionSummary
    }
...
function formatBreakdownPrice(
  ...
  const amount =
    ...
    field.unit === 'image'
      ...
      : `${symbol}${(value * rate * (taskPriceOptions?.groupRatioMultiplier ?? 1)).toFixed(4)}`
  const threshold =
    tier && isTaskBreakdownTier(tier)
      ? tier.freeThresholds?.[field.id]
      : undefined
  const thresholdText = threshold
    ? ` (${t('first {{count}} free', { count: threshold, defaultValue: `前${threshold}张免费` })})`
    : ''
  if (field.unit === 'second') return `${amount}/${t('s')}`
  if (field.unit === 'count') {
    return `${amount}/${taskUsageUnitLabel(field, language, t('unit'))}${thresholdText}`
  }
...
function describeCondition(
  ...
  if (cond.source === SOURCE_TIME) {
    ...
    const isZh = t('Hour') === '小时'

    if (cond.timeFunc === 'weekday') {
      const names = isZh ? WEEKDAY_NAMES_ZH : WEEKDAY_NAMES_EN
      if (cond.mode === MATCH_RANGE) {
        const s = Number(cond.rangeStart)
        const e = Number(cond.rangeEnd)
        if (s === 1 && (e === 6 || e === 5)) {
          return `${isZh ? '周一至周五' : 'Mon - Fri'} (${tz})`
        }
        const startName = names[s] || cond.rangeStart
        const endName = names[e > 0 && e <= 7 ? (e === 7 ? 0 : e - 1) : e] || cond.rangeEnd
        return `${startName} ~ ${endName} (${tz})`
      }
      const valNum = Number(cond.value)
      const dayName = names[valNum] || cond.value
      const opMap: Record<string, string> = {
        [MATCH_EQ]: '=',
        [MATCH_GTE]: '≥',
        [MATCH_LT]: '<',
      }
      return `${fn} ${opMap[cond.mode] || '='} ${dayName} (${tz})`
    }

    if (cond.timeFunc === 'hour') {
      const pad2 = (v: string | number) => String(v).padStart(2, '0')
      if (cond.mode === MATCH_RANGE) {
        return `${pad2(cond.rangeStart)}:00 ~ ${pad2(cond.rangeEnd)}:00 (${tz})`
      }
      const opMap: Record<string, string> = {
        [MATCH_EQ]: '=',
        [MATCH_GTE]: '≥',
      ...
      return `${fn} ${opMap[cond.mode] || '='} ${pad2(cond.value)}:00 (${tz})`
    }

    if (cond.mode === MATCH_RANGE) {
      return `${fn} ${cond.rangeStart} ~ ${cond.rangeEnd} (${tz})`
    }
    const opMap: Record<string, string> = {
      [MATCH_EQ]: '=',
      [MATCH_GTE]: '≥',
            ...
            tiers.some(
              (tier) =>
                ...
                (Number(tier.unitPrices[field] || 0) > 0 ||
                  Number(tier.freeThresholds?.[field] || 0) > 0)
            )
        )
