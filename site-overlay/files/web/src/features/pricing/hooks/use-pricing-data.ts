...
export function usePricingData(enabled = true) {
  ...
  useEffect(() => {
    queryClient.removeQueries({
      queryKey: ['pricing'],
      predicate: (query) =>
        query.queryKey[1] !== (userId ?? 'anonymous') ||
        query.queryKey[2] !== (sessionId ?? 'no-session'),
    })
  ...
  const models = useMemo(() => {
    ...
    return data.data.map((model) => {
      ...
      return {
        ...
        model_group_ratio: userId == null ? undefined : data.model_group_ratio?.[model.model_name],
        model_discount: userId == null ? undefined : data.model_discounts?.[model.model_name],
      }
    })
  }, [data, userId])
