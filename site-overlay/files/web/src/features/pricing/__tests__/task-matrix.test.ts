...
describe('task expression free allowance recognition', () => {
  test('parses a count-field price that starts after a free allowance', () => {
    ...
    assert.deepEqual(
      parseTaskTiersFromExpr(
        'tier("base", (u("input_images") > 5 ? (u("input_images") - 5) * 0.05 : 0) + u("seconds") * 0.5)',
        schema
      ),
      [
        {
          ...
          conditions: [],
          constant: 0,
          unitPrices: { input_images: 0.05, seconds: 0.5 },
          freeThresholds: { input_images: 5 },
        },
      ]
    )
  })
