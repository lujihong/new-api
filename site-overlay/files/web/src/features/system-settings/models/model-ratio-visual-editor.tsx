...
>(function ModelRatioVisualEditor(
  ...
  const saveModelPricingMutation = useSaveModelPricing()
  const fullPricingQuery = useModelPricing([], true)
  ...
  const models = useMemo(() => {
    ...
    const modelNames =
      filterMode === 'unset'
        ? new Set(
            (candidateModelNames ?? []).filter((n) => !deletedNames.has(n))
          )
        : new Set(
            [...savedByName.keys(), ...draftByName.keys()].filter(
              (n) => !deletedNames.has(n)
            )
          )
  ...
  const confirmDelete = useCallback(
    async (name: string) => {
      try {
        const entry = fullPricingQuery.data?.entries.find(
          (e) => e.model_name === name
        )
        await saveModelPricingMutation.mutateAsync([
          {
            model_name: name,
            expected_version:
              entry?.version ?? fullPricingQuery.data?.empty_version ?? '',
            pricing: {},
            reset: true,
          },
        ])
        ...
        toast.success(t('Model pricing deleted successfully'))
      } catch (error) {
        handleServerError(error)
      }
    },
    [
      fullPricingQuery.data,
      saveModelPricingMutation,
      modelPrice,
      modelRatio,
      cacheRatio,
      createCacheRatio,
  ...
  return (
    <div className='flex min-h-0 flex-1 flex-col gap-4'>
      ...
      <ConfirmDialog
        ...
        desc={t(
          'Are you sure you want to delete the pricing for {{name}}? It will be removed from configured models.',
          { name: modelToDelete ?? '' }
        )}
        confirmText={t('Delete')}
        destructive
        isLoading={saveModelPricingMutation.isPending}
        handleConfirm={async () => {
          if (!modelToDelete) return
          await confirmDelete(modelToDelete)
        }}
      />
    </div>
  )
